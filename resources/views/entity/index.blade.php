...
@section('content')
    ...
    @foreach($list as $element)
       <div class="card mb-3">
           <div class="card-header" data-entity-unique-name="{{ $type }}" data-uid="{{ $element->getUid() }}">{{ $element->getUniqueName() }}</div>
           <div class="card-body">
           ...
           <div class="card-footer" data-entity-interval="{{ $type }}" data-uid="{{ $element->getUid() }}">{{ $element->getHatInterval() }}</div>
       </div>
    @endforeach
